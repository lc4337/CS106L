...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap<K, M, H> &other) : HashMap{kDefaultBuckets} { 
...
HashMap<K, M, H>::HashMap(HashMap<K, M, H> &&other){  
    ...
    //it's slower, and I don't know why, damn !
    // {
    //     _buckets_array = std::move(other._buckets_array);
    //     other._buckets_array.resize(bucket_count()); //don't understand why, but it just passed the test :(
    // }

    //still don't understand why this can pass the test in O(1), even faster than the above one
...
HashMap<K, M, H>& HashMap<K, M, H>::operator=(HashMap<K, M, H> &&other) {
    ...
    // other._size = 0 will make it failed on test F ( why is that ???
    // other._size = 0;
    return *this;
}
/* end student code */
